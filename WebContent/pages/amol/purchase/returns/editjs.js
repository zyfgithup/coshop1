...
{  var sid=document.getElementById("sid").value;
  ...
  if(storageid!=null){
	...
	if (navigator.userAgent.indexOf("MSIE")>0){//IE判断
	   var cus=window.showModalDialog(URL_PREFIX+"/pages/amol/base/product/pselector.jsp?sid="+sid+"&storageid="+storage,null,"dialogWidth=800px;resizable: Yes;");
	   if(cus!=null){
		     ...
		     var cRow = allRows[1].cloneNode(true);
		      ...
		      var tab13 = document.getElementsByName("stock") ;
		      tab13[rows-2].value=cus.outprice;
		      var tab14 = document.getElementsByName("pcode") ;
			    tab14[rows-2].value=cus.code; 
			    showUnits(tab3[rows-2]);
	      }
	   }else{
			window.open(URL_PREFIX+"/pages/amol/base/product/pselector.jsp?sid="+sid+"&storageid="+storage,"","width=800px,height=520,menubar=no,toolbar=no,location=no,scrollbars=no,status=no,modal=yes");
...
function edit(r){
	...
	if (navigator.userAgent.indexOf("MSIE")>0){//IE判断
	 ...
	 if(cus!=null){
		...
		var tab13 = document.getElementsByName("stock") ;
	      tab13[r.rowIndex-1].value=cus.outprice;
...
/**
 * 验证数量必须为数字，并且根据比率算出基本数量，数量必须小于入库单数量
 */
function yzzs(count){ 
	var r=count.parentNode.parentNode;
	...
	if(isNaN(count.value)||count.value-0<0){
     ...
     for(var i=1;i<pids.length;i++){
		   	if(pids[i].value==pids[r.rowIndex-1].value){
		   		sl=Number(sl)+Number(tab1[i].value);
		   	}
	   	} 
      
     //
     if(stock[r.rowIndex-1].value !="" && stock[r.rowIndex-1].value-sl<0){
    	 alert("出库数量大于库存数量"+stock[r.rowIndex-1].value+"！");
    		count.value="0";
    	   	tab1[r.rowIndex-1].value=0;
     }
     }
