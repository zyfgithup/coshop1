...
function saveyz(){
	...
	var ncount = document.getElementsByName("ncount");
     ...
     var samount = document.getElementById("rttao");//本次退货货款金额
     samount.value = amount;
     
     var cardamountSales = document.getElementById("cardamountSales").value;//代币卡消费金额【出库单】
     var rttaoSales = document.getElementById("rttaoSales").value;//代币卡已退金额【出库单】
     var spayamountSales = document.getElementById("spayamountSales").value;//代币卡实收金额（回款金额）【出库单】
     if(rttaoSales != null && rttaoSales != "" && rttaoSales.length != 0){
    	 cardamountSales = Number(cardamountSales) - Number(rttaoSales);// - Number(spayamountSales);//代币卡真实消费金额
     }
     
     var stje = Number(cardamountSales) - Number(rttaoSales) - Number(spayamountSales) - Number(samount.value);
     if(Number(stje) < 0){
    	 /*if(Number(samount.value) > Number(stje)){
    		 alert("退款金额已超过最大允许退款金额");
    		 return false;
    	 }else{*/
    		 stje = -(Math.round(stje*100)/100);
        	 samount.value = Number(samount.value) - Number(stje);
    	 //}
     }
     
     if(Number(samount.value) > Number(cardamountSales)){
    	 alert("退货货款金额不能超过【刷卡金额减去已退金额减去实收金额】！");
    	 return false;
     }
...
function selectSalesCard(){
	var cus=window.showModalDialog(URL_PREFIX+"/pages/amol/sales/sales/cardSales/selector.jsp",null,"dialogWidth=55","dialogHeight=300px");
    if(cus!=null){
    	window.location.href="cardReturnUI.do?stId=" + cus.id;
	}
}
</script>
</head>
...
<div class="x-panel-header">销售退货单【代币卡】</div>
...
<s:form action="save" id="save" validate="true" method="POST">
	...
	<table align="center" width="95%">
		<tr>
			<td>
			<div align="left">
				<input name="button" type="button"	value="选择出库单" class="button" onclick="selectSalesCard()">
				&nbsp;&nbsp;
				出库单：
				...
				${rttaoSalesCard }
				元
				<input type="hidden" id="cardamountSales" name="cardamountCard" value="${cardamountCard }"/>
				<input type="hidden" id="spayamountSales" name="spayamountSalesCard" value="${spayamountSalesCard }"/>
				<input type="hidden" id="rttaoSales" name="rttaoSalesCard" value="${rttaoSalesCard }"/>
			</div>
			</td>
		</tr>
	</table>
	...
	<table width="100%" align="left" >
		...
		<tr>
			...
			<td align="left">
				...
				<input type="text" value="<%=Payment.CARD.getName() %>" style="width: 155px;" disabled="disabled"/>
			</td>
			<td align="right" >
				应退金额：
			</td>
  ...
  <tbody id="tbody">
   ...
   <tr align="center" bgcolor="#FfFFFF"> 
      ...
      <td>
      		...
      		<input name="counts" value="<%=s.getTnorootl()%>" type="hidden" size="10" value="0"  readonly="readonly" style="color:#808080;"/></td>
