...
<%@page import="java.util.Date,java.util.List,com.systop.amol.sales.model.Sales,com.systop.amol.sales.utils.Payment,java.text.DecimalFormat"%>
 ...
 */
 function selectkh(){
 	var cus=window.showModalDialog(URL_PREFIX+"/pages/amol/base/customer/selectorHy.jsp",null,"dialogWidth=55","dialogHeight=300px");
 	if(cus!=null){
    	 window.location.href="outCardKToReSiUI.do?customerId=" + cus.id;
 	}
 }
...
<div class="x-panel-header">回款单信息【代币卡】</div>
	...
	<table width="100%" align="left" >
	    <tr>
			...
			<td  class="simple" align="left">
				<input type="hidden" name="model.payment" value="<%= Payment.CARD %>"/>
...
<table width="95%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#D9D9D9">
  ...
  <tr bgcolor="#F3F4F3"  align="center" height="23">
    <td width="100px">销售单号</td>
    <td width="100px">代币卡号</td>
   ...
   <% 
     ...
     %>
      ...
      <tr bgcolor="#FFFFFF">
          ...
          <td style="text-align: center;">
             <%=s.getCardGrant().getCard().getCardNo() %>
          </td>
          <td style="text-align: center;">
	      ...
	      <td style="text-align: right">
	         <%=new DecimalFormat("#0.00").format((s.getSamount() - s.getRttao() - s.getSpayamount())) %>
	         <input type="hidden" name="nopay" style="text-align: right" value="<%=(s.getSamount() - s.getRttao() - s.getSpayamount()) %>"/> 
	      </td>
	      <td style="text-align: center;">
	      	<input type="text" style="text-align: right" name="amount1" id="amount1" size="5" onkeyup ="yzszss(this,<%=(s.getSamount() - s.getRttao() - s.getSpayamount()) %>)" maxlength="9"/>
	      </td>
      </tr>
     <%}}%>
