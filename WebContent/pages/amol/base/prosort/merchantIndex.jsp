...
Ext.onReady(function(){
    ...
    tree = new Tree.TreePanel({
        ...
        loader: new Tree.TreeLoader({
            dataUrl:'${ctx}/base/prosort/prosortTree.do?type=2'
        }),
        listeners: {
...
function editWin(node, type){
	if(node.id == 0 && type != 'add'){
		alert("不允许修改或删除顶级商品类型【" + node.text + "】");
		return;
	}
	typeNow = type;
	if(!win){
		var form = new Ext.form.FormPanel({
			url:'${ctx}/base/prosort/asynchronousSave.do?type=2',
			id:'myForm',
	        //baseCls: 'x-plain',
	        //labelWidth: 35,
	        //defaults: { width: 200 },
...
function remove(id, node){
	...
	if(node.isLeaf()){
		...
		//if(button=='yes'){
			...
			Ext.Ajax.request({
				url:'${ctx}/base/prosort/removeMerSort.do',
				params:
		        {
					'model.id':id
		        },
...
<div class="x-panel-header">
 <table width="100%">
    <tr>
      <td width="80" align="center"><b>商家类型管理</b></td>
	  ...
	  <td align="right">
		<a href="${ctx}/base/prosort/merEdit.do"><img src="${ctx}/images/icons/add.gif" />&nbsp;新建商家类型&nbsp;</a>
 	  </td>
	</tr>
  </table>
</div>
