...
function remove(id){
	if (confirm("确认要删除商品吗？")){
		window.location.href="${ctx}/base/product/groupPurchase/remove.do?model.id=" + id;
	}
}
...
<body>
<div class="x-panel">
<div class="x-panel-header">团购商品管理</div>
<div class="x-toolbar">
<table width="100%">
	<tr>
		...
		<td style="padding-left: 5px; padding-top: 5px; width: 50%"
			align="right"><a href="${ctx}/base/product/groupPurchase/edit.do"><img
...
<ec:table items="items" var="item"
	...
	<ec:row>
		...
		<ec:column width="100" property="tgfxs.name" title="分销商" style="text-align:right"/>
		<ec:column width="120" property="_royaltyRateProvince" title="提成比例（省）" style="text-align:right">
			<c:if test="${item.royaltyRateProvince != null }">
				${item.royaltyRateProvince }%
			</c:if>
		</ec:column>
		<ec:column width="120" property="_royaltyRateCounty" title="提成比例（村）" style="text-align:right">
			<c:if test="${item.royaltyRateCounty != null }">
				${item.royaltyRateCounty }%
			</c:if>
		</ec:column>
