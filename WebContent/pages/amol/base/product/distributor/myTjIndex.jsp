...
<div class="x-toolbar">
	   <table width="100%">
        <tr>
        	<td>
        		<table>
			        <s:form action="myTjIndex" theme="simple">
			        	<td>
			        		商品类型：
			        	</td>
			        	<td align="left">
			...
			src="${ctx}/images/icons/add.gif" />&nbsp;批量操作&nbsp;</a></span><a href="tjEdit.do"><img
...
<ec:table items="items" var="item"
	retrieveRowsCallback="limit" sortRowsCallback="limit" action="myTjIndex.do"
	...
	<ec:row>
		...
		<ec:column width="180" property="_2" title="操作"
			...
			<s:if test="#attr.item.upDownGoodshelf == 0">
		    	<a href="tjupOrDownShelf.do?model.id=${item.id}&model.upDownGoodshelf=1">上架 |</a> 	    	
			</s:if> 
			<s:else>
				<a href="tjupOrDownShelf.do?model.id=${item.id}&model.upDownGoodshelf=0">下架 |</a> 
			</s:else>
