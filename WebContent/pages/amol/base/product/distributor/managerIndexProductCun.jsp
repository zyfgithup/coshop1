...
function remove(id){
	if (confirm("确认要删除商品吗？")){
		window.location.href="${ctx}/base/product/distributor/managerCunremove.do?model.id=" + id;
	}
}
...
<div class="x-toolbar">
	   <table width="100%">
        <tr>
        	<td>
        		<table>
			        <s:form action="managerIndexProductCun" theme="simple">
			        	<td>
			        		商品名称：
			        	</td>
			        	<td align="left">
...
<ec:table items="items" var="item"
	retrieveRowsCallback="limit" sortRowsCallback="limit" action="managerIndexProductCun.do"
	...
	<ec:row>
		...
		<ec:column width="220" property="region.name" title="所属地区"  ellipsis="true">
		...
		<ec:column width="100" property="_0" title="操作"
			...
			<!--<s:if test="#attr.item.belonging == 0">
				<font color="#999999" title="修改商品">上架</font> | 
				<font color="#999999" title="删除">删除</font>
			</s:if>
			<s:if test="#attr.item.belonging == 1">
				<a href="managerZyedit.do?model.id=${item.id}" title="修改商品">编辑</a> | 
...
function partCtr(){
	if (confirm("确认要操作这些商品吗？")){
		window.location.href="${ctx}/base/product/distributor/plUpdownjia.do?type=managerSave&strs=" + strs;
	}
}
</script>
</body>
