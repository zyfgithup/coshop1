...
<body>
<div class="x-panel">
<div class="x-panel-header">特价商品管理【平台】</div>
<div class="x-toolbar">
	   <table width="100%">
        <tr>
        	<td>
        		<table>
			        <s:form action="tjindex" theme="simple">
			        	<td>
			        		商品类型：
			        	</td>
			        	<td align="left">
		...
		<!--  td style="padding-left: 5px; padding-top: 5px; width: 50%"
			align="right"><a href="${ctx}/base/product/distributor/edit.do"><img
...
<ec:table items="items" var="item"
	retrieveRowsCallback="limit" sortRowsCallback="limit" action="tjindex.do"
	...
	<ec:row>
		...
		<ec:column width="100" property="user.name" title="所属商家" ellipsis="true"/>
		...
		<ec:column width="150" property="_0" title="操作"
			...
			<s:if test="#attr.item.upDownGoodshelf == 0">
		    	<a href="tjupOrDownShelf1.do?model.id=${item.id}&model.upDownGoodshelf=1">上架</a> 	    	
			</s:if> 
			<s:else>
				<a href="tjupOrDownShelf1.do?model.id=${item.id}&model.upDownGoodshelf=0">下架</a> 
			</s:else>
			<c:if test="${logonUser.type=='agent' }">
				<a href="tjEdit.do?model.id=${item.id}" title="修改商品"> | 编辑</a> | 
				<a href="tjremove.do?model.id=${item.id}">删除</a>
			</c:if>
		</ec:column>
	</ec:row>
</ec:table>
