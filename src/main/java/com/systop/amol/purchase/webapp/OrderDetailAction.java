...
public class OrderDetailAction extends DefaultCrudAction<PurchaseDetail, OrderDetailManager> {
	@Autowired
   ...
   //结束日期
	...
	public String index() {
		...
		StringBuffer sql = new StringBuffer(
				"from PurchaseDetail pd where pd.purchase.status=0 and pd.purchase.user.id=? and pd.purchase.billType=0");
		List args = new ArrayList();
		args.add(user.getId());
		if (getModel() != null && getModel().getPurchase() != null
				&& !getModel().getPurchase().getSno().trim().equals("")) {
		...
		String nsql = "select sum(pd.count),sum(pd.amount),sum(pd.linkCount) "
			+ sql.toString();
	List list = getManager().query(nsql, args.toArray());
	String countTotal = "";
	String amoutTotal = "";
	String linkCountTotal = "";
	if (list.size() > 0) {
		for (Object o : list) {
			Object[] oo = (Object[]) o;
			if (oo[0] != null) {
			...
			if (oo[2] != null) {
				linkCountTotal = oo[2].toString();
			}
		}
	}

		getRequest().setAttribute("linkCountTotal", linkCountTotal);
