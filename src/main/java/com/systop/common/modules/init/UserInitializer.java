...
public class UserInitializer {
	...
	public void init() {
		...
		try {
			...
			if (!hasAdmin(session)) {
				...
				admin.setBeginningInit(Constants.YES);
				// 是否可用：可用
				...
				for (Role role : UserConstants.SYS_ROLES) {
					if (!hasSysRole(session, role.getName())) {
						session.save(role);
						if (role.getIsSys().equals(UserConstants.USER_TYPE_SYS)) {
	...
	private Dept getTopDept(Session session) {
		List<Dept> list = session.createQuery(
				"from Dept d where d.parent = null").list();
	...
	private boolean hasAdmin(Session session) {
		List<User> list = session.createQuery("from User u where u.loginId='admin'").list();
