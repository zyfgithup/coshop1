...
public class UserDetailService implements UserDetailsService {
	...
	public UserDetails loadUserByUsername(String username) {
		...
		if (userCache != null) { // 从缓存中取得UserDetails
			...
			if (userDetails != null) {
				logger.debug("Get user details from cache {}/{}",
						userDetails.getPassword(), username);
			}

		}
		if (userDetails == null) { // 如果缓存中没有，则查询数据库，然后putInCache
			User user = userManager.findObject(
					"from User u where u.loginId=? and u.status=?", username,
					Constants.STATUS_AVAILABLE);
			if (user == null) {//抛出异常
				throw new UsernameNotFoundException(username);
			}
			logger.debug("password of user '{}/{}'", user.getLoginId(),
					user.getPassword());
			...
			for (int i = 0; i < perms.size(); i++) {
				...
				logger.debug("Auth of user '{}' is '{}'", user.getLoginId(),
