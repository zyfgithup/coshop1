...
public class StrutsUrlImportService {
	...
	public void save() {
		...
		if (!resStrings.isEmpty()) {
			for (Iterator listItr = resStrings.iterator(); listItr.hasNext();) {
				...
				com.systop.common.modules.security.user.model.Resource resource = new com.systop.common.modules.security.user.model.Resource();
	...
	/**
	 * 读取配置文件，找到其中的action的完整的路径，包括后缀. 同时，找到配置文件中的include节点，将包含的配置文件也通过递归的方式读取。
	 * 
	 * @return List of full path of actions.
	 */
	private List readStrutsXml(Resource resource) {
		String postfix = getActionPostfix();
		List list = new LinkedList(); // this List will be modifed frequently,
										// so we use linked list

		try {
			Document doc = this.readDoc(resource);
			List packageElements = doc.selectNodes("/struts/package"); // Packages
																		// in a
																		// file.
			if (packageElements != null) {
				for (Iterator itr = packageElements.iterator(); itr.hasNext();) {
					...
					logger.debug("find package '{}'",
							packageEle.attributeValue("name"));
					...
					if (actionElements != null) {
						for (Iterator actionItr = actionElements.iterator(); actionItr
								.hasNext();) {
							Element actionEle = (Element) actionItr.next();
							String actionName = actionEle
									.attributeValue("name");
							String resString = buildActionPath(namespace,
									actionName, postfix);
			...
			if (includes != null) {
				for (Iterator itr = includes.iterator(); itr.hasNext();) {
					...
					if (incEle != null) {
						...
						logger.debug("Find a included struts file [{}]",
								filename);
	...
	/**
	 * 从Struts的配置文件中，找到被包含（inclided）的struts配置文件， 从struts2.1.2开始，支持批量包含，例如：
	 * 
	 * <pre>
	...
	private Resource getRootConfigFile(String filePath) {
		if (filePath.startsWith(ResourceUtils.CLASSPATH_URL_PREFIX)) {
			filePath = filePath.substring(ResourceUtils.CLASSPATH_URL_PREFIX
					.length());
		}
	...
	private String getActionPostfix() {
		...
		if (rb != null) {
			actionPostfix = ResourceBundleUtil.getString(rb, XPATH_STRUTS_EXT,
					DEFAULT_POSTFIX);
			return actionPostfix;
		}
		return DEFAULT_POSTFIX;
	 ...
	 * @return
	 */
	private String buildActionPath(String namespace, String actionName,
			String actionPostfix) {
		...
		return new StringBuffer(namespace).append(actionName).append(".")
				.append(actionPostfix).append("*").toString();
	}
