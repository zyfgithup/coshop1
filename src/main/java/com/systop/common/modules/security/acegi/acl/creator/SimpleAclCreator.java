...
public class SimpleAclCreator extends AbstractBasicAclCreator {
	...
	protected SimpleAclEntry[] getEntry(Authentication authentication,
		...
		AclObjectIdentity aclIdentity = AclUtils.obtainIdentity(domainInstance,
				defaultAclObjectIdentityClass);
		Assert.notNull(aclIdentity,
				"domainInstance is not supported by this creator");
		...
		for (int i = 0; i < authoritys.length; i++) {
			...
			if (auth.getAuthority().startsWith("ACL_")) {
				SimpleAclEntry aclEntry = new SimpleAclEntry(
						auth.getAuthority(), aclIdentity, parentAclIdentity,
