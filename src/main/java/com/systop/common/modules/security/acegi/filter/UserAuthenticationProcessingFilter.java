			...
			if (httpSession.getAttribute(Constants.USER_IN_SESSION) == null) {
				if (!requiresAuth) {
					...
					if (auth != null && auth.getPrincipal() instanceof UserDetails) {
						...
						Set<Resource> setR = new HashSet<Resource>();
						if(null!=user && null!=user.getRole()) {
							Set<Permission> pSet = user.getRole().getPermissions();
							if (null != pSet) {
								Iterator<Permission> it = pSet.iterator();
								while (it.hasNext()) {
									Permission per = it.next();
									Set<Resource> sr = per.getResources();
									if (null != sr) {
										System.out.println("------------------sr.size()=" + sr.size());
										setR.addAll(sr);
										System.out.println("------------------setR.size()=" + setR.size());
									}
								}
								String resurceStr = "";
								Iterator<Resource> it1 = setR.iterator();
								while (it1.hasNext()) {
									resurceStr += it1.next().getResString() + ",";
								}
								System.out.println("------------------resurceStr=" + resurceStr);
								if (null != resurceStr) {
									httpSession.setAttribute("resurceStr", resurceStr.substring(0, resurceStr.length() - 1));
								}
							}
						}
						// 更新用户登录信息，例如登录次数等
