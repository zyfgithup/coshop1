...
public class RestorePasswordAction extends BaseAction {
	...
	/**
	 * Body of the password mail, {user} means username, and {pwd} means new
	 * password. Example:<br>
	 * 
	 * <pre>
	 * Dear {user}:
	 ...
	 * </pre>
	 * 
	 * or a freemarker template file. and the file must be at the
	 * classpath:tempates/simple.
	 * 
	 */
	...
	public String sendPasswordMail() {
		try {
			...
			User user = (User) userManager.getDao().findObject(
					"from User u where u.email=? and u.loginId=?", emailAddr,
					loginId);
			if (user == null) {
	...
	/**
	 * Retrieve the mail body. If the <code>templateRender</code> is not null,
	 * getMailBody method will render the freemarker template as the body.
	 * Otherwise, Use <code>bodyTemplate</code> as the template and replate the
	 * {user} and {pwd} sign in it.
	 * 
	...
	private String getMailBody(User user, String password) {
		if (templateRender == null) {
			return bodyTemplate.replace("{user}", user.getLoginId()).replace(
					"{pwd}", password);
		}
		...
		Map data = ReflectUtil.toMap(user, new String[] { "loginId", "email" },
				false);
